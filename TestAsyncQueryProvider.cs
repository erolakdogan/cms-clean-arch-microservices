...
namespace Tests.Shared.AsyncQuerying;
...
public sealed class TestAsyncQueryProvider<TEntity> : IAsyncQueryProvider
{
    ...
    public Task<TResult> ExecuteAsync<TResult>(Expression expression, CancellationToken _) => Task.FromResult(Execute<TResult>(expression));
}
...
public sealed class TestAsyncEnumerable<T> : EnumerableQuery<T>, IAsyncEnumerable<T>, IQueryable<T>
{
    ...
    public IAsyncEnumerator<T> GetAsyncEnumerator(CancellationToken _) => new TestAsyncEnumerator<T>(this.AsEnumerable().GetEnumerator());
